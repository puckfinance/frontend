...
export async function GET(request: NextRequest) {
  ...
  const upstreamUrl = `${backendUrl}/api/v1/ai/backtest/stream?symbol=${encodeURIComponent(symbol)}&date=${encodeURIComponent(date)}&timeframe=${encodeURIComponent(timeframe)}`;
